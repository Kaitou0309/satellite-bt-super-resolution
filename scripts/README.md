...
- `evaluation/geo_plotting.py`: shared plotting helper that uses longitude/latitude axes when matching coordinate datasets exist.
...
Model YAML files are the source of truth for released architecture, checkpoint, and normalization metadata. MW-SR-GAN inference loads only its RRDN generator; discriminator BatchNorm is a training configuration.
